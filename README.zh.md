...
自研包，基于官方遥测 seam（`@deepseek-ai/dsh-session-telemetry`）。设计上参考了社区插件 [`dsh-plugin-langfuse`](https://github.com/linyp/dsh-plugin-langfuse)（MIT，Copyright (c) 2026 Yupeng Lin，署名保留在 [NOTICE](NOTICE)），实现完全独立。与社区版相比有四个刻意差异：
