...
This is an in-house package built on the harness's public telemetry seam (`@deepseek-ai/dsh-session-telemetry`) as an alternative backend to the official OTLP-*logs* exporter. The design was developed with reference to the community plugin [`dsh-plugin-langfuse`](https://github.com/linyp/dsh-plugin-langfuse) (MIT, Copyright (c) 2026 Yupeng Lin — attribution kept in [NOTICE](NOTICE)); the implementation is independent. It differs from that project in four deliberate ways:

1. **Content-Length transport.** Spans are delivered as one write with an explicit `Content-Length` header — never `Transfer-Encoding: chunked`. Some Langfuse deployments sit behind front gateways that mangle chunked POST bodies; Langfuse's OTLP route then fails them with `400 Failed to parse OTel JSON Trace` even though the bytes were valid. (The OTel SDK's own HTTP exporter always sends chunked and offers no switch.) With this package no `node_modules` patch is needed on either side.
...
The commands assume the installed `dsh` CLI. Running the official harness from a source checkout? Run each of them as `pnpm dsh …` from the checkout root (after `pnpm run build`) — same commands, same `web` profile.
...
| `processor` | Passed verbatim to `BatchSpanProcessor` (`scheduledDelayMillis`, `maxQueueSize`, `maxExportBatchSize`, …); batching and queueing are the SDK's documented behavior. Retries (bounded exponential backoff) live in this package's exporter. |
...
| `maxAttributeChars` | Serialized-payload ceiling per span attribute (default 16384); longer payloads are clipped with an `…[truncated]` marker while the canonical session log keeps the full bytes. |
...
| `step/start` / `step/end` + `request/header` + `assistant/message` | **generation** — model, provider, output, `gen_ai.usage.*` tokens (input/output/cache-read/reasoning), optional input |
...
- **Plain OTel traces SDK, not the Langfuse SDK.** The official `session-telemetry-otel` backend exports OTLP **logs**; Langfuse's OTLP endpoint accepts **traces only**. Spans are built with the OTel SDK (`BasicTracerProvider` → `BatchSpanProcessor`) and serialized with `@opentelemetry/otlp-transformer`; a custom `SpanExporter` delivers the JSON with `Content-Length`.
- **A folding projection.** The seam hands over a flat record stream; Langfuse needs a tree. `SessionSpanFolder` folds records into spans keyed by `(session.id, turn, step)`. Timestamps always come from the record's `time`, never the wall clock, so live capture and `FEEDBACK_ONLY` canonical-log replay produce identical trees. `seq` gaps are routine, never a loss signal. Unknown event types land as span events on the open turn. Force-end sweeps close still-open spans on a next `turn/start` with an open predecessor, on the session's ops `shutdown` record, and on backend shutdown.
- **Delivery semantics: at-most-once handoff, duplicates possible** (inherited from the seam). This package adds bounded retries inside its exporter (backoff, 6 attempts by default) so the loss window is the crash window only; receivers correlate on `langfuse.session.id` + `dsh.turn_idx` + `dsh.event_seq`.
...
None. This plugin only observes the session stream through the telemetry seam and hands folded spans to the OTel SDK; it never contributes to a model request, never assembles or sends a provider request.
...
The suite covers config validation, the folding projection (hierarchy, attributes, error statuses, force-end sweeps, first-chunk dedupe, lineage links, clipping), the transport (`Content-Length` present, no `Transfer-Encoding`, retry/backoff behavior), score pushes, and a full seam integration that drives a real `SessionTelemetryCoordinator` over a Cordis context and asserts the wire payload.

A real-deployment smoke against a self-hosted Langfuse (v3.171.0) was run from a sandbox profile: one headless task produced a `dsh turn 1` trace with a GENERATION observation carrying model, usage (including cache-read tokens), completion-start time, and output — delivered direct, without any gateway-side or `node_modules` patches.
